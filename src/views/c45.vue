...
let scene, camera, renderer, light, controls, material;
...
export default {
  ...
  methods: {
    ...
    addParticle() {
      ...
      var geometry0 = this.createParticle(
        fireRadius,
        fireHeight,
        particleCount
      );
      ...
      // texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
      var material0 = this.createMaterial(this.getTexture());
      // material0.setPerspective( camera.fov, height );
      material0.uniforms.heightOfNearPlane.value = Math.abs(
        window.innerHeight /
    ...
    createParticle(radius, height, particleCount, rowLen = 1 / 4) {
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        sprite[i] = rowLen * ((Math.random() * 4) | 0);
      ...
      geometry.addAttribute("randam", new THREE.BufferAttribute(randam, 1));
      geometry.addAttribute("sprite", new THREE.BufferAttribute(sprite, 1));
      return geometry;
    },
    createMaterial(texture, color, size) {
      color = color || new THREE.Color(0xff2200);
      size = size || 0.4;
      const uniforms = {
